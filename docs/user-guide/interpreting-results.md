...
Every scenario also writes `trait.simple_ltm.parquet`, a binary
liability-threshold phenotype generated by `simace.phenotype.threshold`.
...
`stats_report.yaml` and `plotting_sample.parquet`, so separate Simple LTM
